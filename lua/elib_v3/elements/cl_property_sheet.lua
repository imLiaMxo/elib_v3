...
function PANEL:Setup(text, propertySheet, panel)
	...
	Elib.SetFont("UI.Tab")
	self:SetWide(Elib.GetTextSize(text) + Elib.Scale(16))
end
...
function PANEL:DoRightClick()
	...
	local tabs = vgui.Create("Elib.Menu", self)
...
function PANEL:Paint(w, h)
	self.Color = Elib.LerpColor(FrameTime() * 12, self.Color, (self:IsActive() or self:IsHovered()) and self.SelectedCol or self.BackgroundCol)
	self.TextColor = Elib.LerpColor(FrameTime() * 12, self.TextColor, (self:IsActive() or self:IsHovered()) and self.SelectedTextCol or self.UnselectedTextCol)

	Elib.DrawRoundedBoxEx(Elib.Scale(6), 0, 0, w, h, self.Color, true, true)
	Elib.DrawSimpleText(self:GetText(), "UI.Tab", w * .5, h * .5, self.TextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end

vgui.Register("Elib.Tab", PANEL, "Elib.Button")
...
function PANEL:Init()
	...
	self.tabScroller:SetOverlap(Elib.Scale(5))
	self.tabScroller:Dock(TOP)
	self.tabScroller:DockMargin(Elib.Scale(3), 0, Elib.Scale(3), 0)

	self:SetFadeTime(0.1)
	self:SetPadding(Elib.Scale(8))
...
function PANEL:AddSheet(label, panel, material, noStretchX, noStretchY, tooltip)
	if not IsValid(panel) then
		ErrorNoHalt("Elib.PropertySheet:AddSheet tried to add invalid panel!")
		debug.Trace()
		return
	end
	...
	sheet.Tab = vgui.Create("Elib.Tab", self)
	...
	sheet.Panel:SetPos(self:GetPadding(), Elib.Scale(24) + self:GetPadding())
